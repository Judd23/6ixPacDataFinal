...
if (all(map_lgl(df[indicators], is.ordered))) {
  ...
  if (!inherits(pc, "try-error")) {
    cor_mat <- pc$rho
    write_csv(as_tibble(cor_mat), file.path(out_dir, "cfa_success_polychoric_correlations.csv"))
  }
} else {
  cor_mat <- cor(df %>% select(all_of(indicators)) %>% mutate(across(everything(), ~ as.numeric(as.character(.)))), use = "pairwise.complete.obs")
  write_csv(as_tibble(cor_mat), file.path(out_dir, "cfa_success_pearson_correlations.csv"))
}
...
R_SCRIPTcd "/Users/jjohnson3/Documents/GitHub/6ixPacDataFinal"
pwd
cat > src/r/cfa/cfa_success_full.R <<'R_SCRIPT'ls -l src/r/cfa/cfa_success_full.R
ls -l data/interim/df_subset.csv
head -n 8 data/interim/df_subset.csv
cd "/Users/jjohnson3/Documents/GitHub/6ixPacDataFinal"
pwd
pwd
ls -la

# (optional) ensure build tools if R needs to compile source packages
xcode-select --install
# if you use Homebrew and need gfortran support:
brew install gcc

# Install the R packages (will try CRAN binaries where available)
