...
pkg <- c("tidyverse", "lavaan", "semTools", "psych", "mice")
...
library(psych)
library(mice)
# semPlot is optional (plotting); we'll use it only if available to avoid installation failures
...
## Plot path diagram (png) — optional; skip if semPlot is not installed
plot_file <- file.path(out_dir, "cfa_success_path.png")
if (requireNamespace("semPlot", quietly = TRUE)) {
  try({
    png(plot_file, width = 800, height = 600)
    semPlot::semPaths(fit, what = "std", layout = "tree", residuals = FALSE, style = "ram", edge.label.cex = 1.2)
    dev.off()
  }, silent = TRUE)
} else {
  message("Package 'semPlot' not installed; skipping path diagram (install semPlot to enable plotting)")
}
